...
2. **Double-click `PLAY.bat`** at the repo root.
...
First install or big dependency changes: run **`PLAY.bat /FULL`** (or `INSTALL.bat` once) to copy the full pack folder.
...
## PLAY.bat vs symlink mode

| Method | Best for | Notes |
|--------|----------|-------|
| **`PLAY.bat`** | Default dev loop | Robocopy changed P1 folders; safe; works without admin |
...
Maintainers use **`QUICK_UPDATE.bat`** or **`tools/quick-release.ps1`** to bump version, rebuild zips, push, and publish.
...
- [GROK_INTEGRATION.md](GROK_INTEGRATION.md) — Grok Build + Cursor dual-agent setup
